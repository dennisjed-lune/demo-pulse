...
(function () {
  ...
  function go(view) {
    ...
    if (view === 'dashboard') { refreshDashboard(); setTimeout(renderTree, 60); }
  ...
  var stackGran = 'monthly', catDaily = null, catDailyByBank = {};
  function ensureCatDaily() {
    if (catDailyByBank[bankFilter]) { catDaily = catDailyByBank[bankFilter]; return; }
    var idx = {}; D.spendLegend.forEach(function (c, i) { idx[c] = i; });
    var cd = D.spendLegend.map(function () { var a = [], i; for (i = 0; i < SPAN_DAYS; i++) a.push(0); return a; });
    var fx = D.fx || {}, tx = D.transactions, i;
    for (i = 0; i < tx.length; i++) { var r = tx[i]; if (!r[2]) continue; if (bankFilter !== 'all' && r[12] !== bankFilter) continue; var ci = idx[r[4]]; if (ci === undefined) continue; cd[ci][dayIdx(r[5])] += Math.abs(amtVal(r[6])) * (fx[r[7]] || 1); }
    catDailyByBank[bankFilter] = cd; catDaily = cd;
  }
  ...
  var treeEntity = 'categories', treeMetric = 'spend', bankFilter = 'all';
  function parseNum(s) { return parseFloat(String(s).replace(/[^0-9.]/g, '')) || 0; }
  function bankSrc() { return (D.bankAgg && D.bankAgg[bankFilter]) || { categories: D.categories, brands: D.brands, kpi: D.kpi }; }
  function treeData() {
    var src = bankSrc();
    if (treeEntity === 'categories') return src.categories.map(function (r) { return { name: r[0], color: r[1], spend: parseNum(r[2]), transactions: parseNum(r[3]), customers: parseNum(r[4]) }; });
    return src.brands.map(function (r) { return { name: r[0], color: r[1], spend: parseNum(r[3]), transactions: parseNum(r[4]), customers: parseNum(r[5]) }; });
  }
  ...
  // bank / source toggle — filters KPIs, treemap and spend-over-time to a bank (or all)
  function renderDashKpis() {
    var k = bankSrc().kpi || D.kpi;
    setTxt('#kpi-customers', fmtCount(k.customers)); setTxt('#kpi-transactions', fmtCount(k.transactions));
    setTxt('#kpi-spend', fmtSpend(k.spend)); setTxt('#kpi-spendcust', fmtSpend(k.spendPerCustomer));
    var lbl = bankFilter === 'all' ? 'across all linked banks' : bankFilter === 'ADIB' ? 'ADIB (on-us) only' : bankFilter + ' (off-us) only';
    setTxt('#dash-source-note', lbl);
  }
  function renderShare() {
    var el = $('#sow-bars'); if (!el || !D.shareOfWallet) return;
    var order = D.banks || ['ADIB', 'FAB', 'Emirates NBD'];
    var items = order.map(function (bk) { return { label: bk, value: D.shareOfWallet[bk] || 0 }; }).sort(function (a, b) { return b.value - a.value; });
    el.innerHTML = items.map(function (x) {
      var pct = x.value * 100;
      return '<div class="lh-distrow"><span class="dl">' + x.label + (x.label === 'ADIB' ? ' · on-us' : ' · off-us') + '</span><span class="dbar"><i style="width:' + pct.toFixed(1) + '%;background:' + (x.label === 'ADIB' ? 'var(--accent)' : 'var(--c-services)') + '"></i></span><span class="dv">' + pct.toFixed(pct < 10 ? 1 : 0) + '%</span></div>';
    }).join('');
    setTxt('#sow-sub', 'opted-in panel of ' + (D.panelSize || 0).toLocaleString() + ' customers');
  }
  function refreshDashboard() { renderDashKpis(); renderTree(); renderStack(); renderShare(); }
  $$('#bank-seg button').forEach(function (b) { b.addEventListener('click', function () { bankFilter = b.getAttribute('data-bank'); refreshDashboard(); }); });
